...
describe('Rise.Color', function() {
    ...
    it("Should properly instantiate", function() {
        var color = new Rise.Color(),
            invalidColor = new Rise.Color("bad");

        color.should.be.an.instanceof(Rise.Color);
        invalidColor.should.be.an.instanceof(Rise.Color);
        color.toHexString().should.be.equal('#000000');
        invalidColor.toHexString().should.be.equal("#000000");
        invalidColor.isValid().should.be.not.ok;

        new Rise.Color('red', {
            format: 'hex'
        }).toString().should.be.equal('#ff0000');
    ...
    it("Should properly parse/convert HSL", function() {
        ...
        new Rise.Color('hsla 251 100 0.38 0.5').toHslString().should.be.equal('hsl(251, 100%, 38%, 0.5)');
        new Rise.Color({
            h: 251,
            s: 100,
            l: 0.38
    ...
    it("Should properly return string", function() {
        var redColor = Rise.Color.fromRatio({
                r: 255,
                g: 0,
                b: 0,
                a: 0.5
            ...
            transparentColor = Rise.Color.fromRatio({
                r: 255,
                g: 0,
                b: 0,
                a: 0
        ...
        redColor.getFormat().should.be.equal('name');
        redColorHex.getFormat().should.be.equal('hex');

        redColor.toString().should.be.equal('rgba(255, 0, 0, 0.5)');
        redColorHex.toString().should.be.equal('rgba(255, 0, 0, 0.5)');

        redColor.toString("hex").should.be.equal('#ff0000');
        redColor.toString("hex6").should.be.equal('#ff0000');
        redColor.toString("hex3").should.be.equal('#f00');
        redColor.toString("name").should.be.equal('#ff0000');

        redColor.toName().should.be.not.ok;

        redColorHex.toString().should.be.equal('rgba(255, 0, 0, 0.5)');
        transparentColor.toString().should.be.equal('transparent');

        redColorHex.setAlpha(0);
        redColorHex.toString().should.be.equal('rgba(255, 0, 0, 0)');
    });

    it("Should properly lighten/brighten/darken/desaturate/saturate/greyscale", function() {
    ...
    it("Should properly check for equals colors", function() {
        ...
        Rise.Color.equals("#ff0000", "rgba(255, 0, 0, .1)").should.be.ok;
        Rise.Color.equals("ff0000", "#ff0000").should.be.ok;
        Rise.Color.equals("#f00", "#ff0000").should.be.ok;
        Rise.Color.equals("f00", "#ff0000").should.be.ok;
        Rise.Color.equals("#ff0000", "#00ff00").should.be.ok;
